    ...
    case "RESTORE_BOARDS": {
      return {
        ...
        boards: action.payload.boards.map((b) => {
          const existing = state.boards.find((s) => s.boardId === b.boardId && s.alias === b.alias);
          return {
            boardId: b.boardId,
            alias: b.alias,
            selectedMcu: b.selectedMcu,
...
export function McuProvider({ children }: McuProviderProps) {
  ...
  // (load/preset/reset) or historyEpoch changes (undo/redo).
  const lastEpochRef = useRef(configState.configEpoch);
  const lastHistoryEpochRef = useRef(configState.historyEpoch);

  const { configEpoch, historyEpoch, document: configDocument } = configState;

  useEffect(
    function restoreBoardsOnDocumentLoadEffect() {
      if (configEpoch === lastEpochRef.current && historyEpoch === lastHistoryEpochRef.current) return;
    ...
    [configEpoch, historyEpoch, configDocument],
  );
    ...
    function autoPopulateBoardPinsEffect() {
      ...
      for (const instance of state.boards) {
        ...
        if (instance.board.aliases && Object.keys(instance.board.aliases).length > 0) {
          ...
          const existing = sections.find(
            (s) =>
              isBoardPinsSection(s.definitionId) &&
          ...
          if (existing) {
            const header =
              existing.definitionId === BOARD_PINS_SECTION
                ? BOARD_PINS_SECTION
                : `${BOARD_PINS_SECTION} ${existing.instanceName}`;
            configDispatch({ type: "REMOVE_SECTION", payload: { header }, skipHistory: true });
          }

          const newSection: import("@klipperforge/klipper-config").SectionInstance = {
            definitionId: instance.alias === "" ? BOARD_PINS_SECTION : BOARD_PINS_NAMED_SECTION,
          ...
          configDispatch({ type: "SET_SECTION", payload: newSection, skipHistory: true });
        } else {
          ...
          if (autoGenerated) {
            const header =
              autoGenerated.definitionId === BOARD_PINS_SECTION
                ? BOARD_PINS_SECTION
                : `${BOARD_PINS_SECTION} ${autoGenerated.instanceName}`;
            configDispatch({ type: "REMOVE_SECTION", payload: { header }, skipHistory: true });
          }
        }
      }
      ...
      for (const prevInstance of prev) {
        ...
        const autoGenerated = configDocumentRef.current.sections.find(
          (s) => isBoardPinsSection(s.definitionId) && s.meta?._boardSource === prevInstance.board?.id,
        );
        if (autoGenerated) {
          const header =
            autoGenerated.definitionId === BOARD_PINS_SECTION
